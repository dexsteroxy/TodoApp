      ...
      {todoList.length > 0 ? (
        <Container className="py-10">
          <div className="max-w-2xl mx-auto bg-white text-slate-950 py-10 px-6 rounded-md shadow-lg shadow-slate-400">
            <p className="text-xl font-semibold mb-5">All Todos </p>
            {todoList.length > 0 && (
              <div>
            ...
            {todoList.length > 1 && (
              <div className="w-full flex items-center justify-end mt-4">
                <button
                  onClick={() => dispatch(resetTodo() ) && toast.success("removed Successfully!")}
        ...
        <div className="flex flex-col items-center py-10 gap-y-2">
          ...
          <Link href={"/"} className=" bg-white p-2 rounded-md text-black shadow-md shadow-slate-400 hover:text-red-600 duration-300 hover:shadow-none  font-semibold">Add todo</Link>
        </div>
      )}
      <Toaster/>
    </>
